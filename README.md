...
hosts/
  nixos-laptop/                shared physical-machine facts for the laptop --
                                 not itself a buildable host (no default.nix)
  ...
  nixos-laptop-gnome/           one NixOS system config: the laptop, booted into GNOME
    default.nix                  imports ../nixos-laptop/{common,hardware-configuration}.nix
                                  plus modules/nixos/desktop/{gdm,gnome}.nix
  nixos-laptop-kde/              same laptop, booted into Plasma instead -- separate
    default.nix                   nixosConfigurations entry, not a session picker; switch
                                   between them with `nixos-rebuild switch --flake .#<host>`
home/<name>/                  one Home Manager config per person/context
  default.nix                  DE-agnostic base (dev tools, shared terminal, browsers, ...)
  gnome.nix, kde.nix           thin: default.nix + one DE's home-manager desktop module
modules/
  nixos/                      reusable NixOS-level pieces
    common.nix                 applies to every host
    desktop/                   gnome.nix, plasma.nix (one DE each, never both on one host)
                                 gdm.nix / sddm.nix, each DE's own native login manager
...
GNOME and Plasma are deliberately **not** installed on the same host
generation — running both together caused real cross-DE bleed (Plasma's
`kde-gtk-config` overwriting GNOME's GTK icon theme, a shared
`services.flatpak.enable` silently pulling KDE Discover into the picture,
SDDM lacking GDM's native keyring integration). Trying either DE now means
building/switching to the other `nixosConfigurations` entry, not logging
into a different session on one shared system.
...
- **`hosts/` and `home/` are thin.** Each file there is just "which modules
  ...
  from the same shelf instead of copy-pasting. `hostname` is the one setting
  that's *deliberately* not shared even between two configs for the same
  physical machine (`nixos-laptop-gnome` / `nixos-laptop-kde`) — it's what
  tells you, at a glance, which DE-flavored generation you're actually
  booted into.
